...
- (void) _foundNetworks {
	...
	for(NSUInteger i = 0; i < [self.simulatedNetworks count]; i++) {
		...
		[self insertCell:^(XHTableViewCell *staticContentCell, UITableViewCell *cell, NSIndexPath *indexPath) {
...
- (void) viewDidLoad {
	...
	[self addSection:^(XHTableViewSection *section, NSUInteger sectionIndex) {
		[section addCell:^(XHTableViewCell *staticContentCell, UITableViewCell *cell, NSIndexPath *indexPath) {
			...
			cell.accessoryView = safeSelf.wifiSwitch;
		}];
	}];
	...
	[self addSection:^(XHTableViewSection *section, NSUInteger sectionIndex) {
		[section addCell:^(XHTableViewCell *staticContentCell, UITableViewCell *cell, NSIndexPath *indexPath) {
			...
			cell.accessoryView = safeSelf.askToJoinSwitch;
		}];
	}];
